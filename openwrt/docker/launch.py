...
class OpenWRT_vm(vrnetlab.VM):
    def __init__(self, username, password, conn_mode, nics, hostname, packet_repository_domains, packet_repository_dns_server, packages, lan_ip, lan_netmask):
        for e in os.listdir("/"):
            if re.search(".img$", e):
                disk_image = "/" + e
    ...
    def vm_stop_start_rm_tc_rules(self):
        ...
        # List all interfaces
        interfaces = [iface for iface in os.listdir("/sys/class/net/") if iface.startswith("eth") or iface.startswith("tap")]
    ...
    def bootstrap_spin(self):
        ...
        if os.path.exists(overlay_target):
            ...
            overlay_version = os.path.basename(overlay_target).split('-')[1]  # Extract overlay version

            if base_version == overlay_version:
                self.logger.info(f"Overlay version matches base image ({base_version}), no update needed.")
            else:
                self.logger.warning(f"Overlay image version mismatch: Base({base_version}) vs Overlay({overlay_version})")
            ...
            if os.path.exists(overlay_img):
                ...
                self.logger.info(f"Overlay image moved: {overlay_img} → {overlay_target}")
                if os.path.exists(overlay_img) and not os.path.islink(overlay_img):
                    self.logger.info(f"Removing existing file at {overlay_img} before creating symlink.")
                    os.remove(overlay_img)
                # Ensure the symlink is created only if necessary
                if not os.path.islink(overlay_img):
                    self.logger.info(f"Symlink does not exist, creating: {overlay_img} → {overlay_target}")
                    os.symlink(overlay_target, overlay_img)
                    self.vm_stop_start_rm_tc_rules()

                elif os.readlink(overlay_img) != overlay_target:
                    self.logger.info(f"Symlink is incorrect, updating: {overlay_img} → {overlay_target}")
                    os.remove(overlay_img)
                    os.symlink(overlay_target, overlay_img)
                    self.vm_stop_start_rm_tc_rules()
                else:
    ...
    def configure_mgmt_interface(self, output):
        ...
        self.get_ready()
        changes_network = 0
        expected_mgmt_address_ipv4 = str(ipaddress.IPv4Interface(self.mgmt_address_ipv4).ip)
        ...
        if "config interface 'mgmt'" not in output:
            ...
            time.sleep(0.5)
            self.tn.write(f"uci set network.mgmt.device='{self.mgmt_interface_device}'\n".encode("utf-8"))
            time.sleep(0.5)
            self.tn.write(b"uci set network.mgmt.proto='static'\n")
            time.sleep(0.5)
            self.tn.write(f"uci set network.mgmt.ipaddr='{ipaddress.IPv4Interface(self.mgmt_address_ipv4).ip}'\n".encode("utf-8"))
            time.sleep(0.5)
            self.tn.write(f"uci set network.mgmt.netmask='{ipaddress.IPv4Interface(self.mgmt_address_ipv4).netmask}'\n".encode())
            time.sleep(0.5)
            if self.mgmt_passthrough_ipv4_address == self.mgmt_address_ipv4:
                # MGMT passthrough active, lets enable IPv6 aswell
                self.tn.write(f"uci set network.mgmt.ip6addr='{self.mgmt_address_ipv6}'\n".encode("utf-8"))
                time.sleep(0.5)
            ...
            time.sleep(0.5)
            current_mgmt_address_ipv4 = self.tn.read_very_eager().decode("utf-8").strip()
            ...
            if current_mgmt_address_ipv4 == expected_mgmt_address_ipv4:
                self.logger.info(f"✅ MGMT IP is already correct: {current_mgmt_address_ipv4}")
            else:
                if self.mgmt_passthrough_ipv4_address != self.mgmt_address_ipv4:
                    # MGMT passthrough active, lets disable IPv6
                    self.tn.write(b"uci del network.mgmt.ip6addr\n")
                    time.sleep(0.5)
                self.logger.warning(f"⚠ MGMT IP mismatch! Current: {current_mgmt_address_ipv4}, Expected: {expected_mgmt_address_ipv4}")
                self.logger.info("🔄 Updating MGMT IP address...")

                self.tn.write(f"uci set network.mgmt.ipaddr='{expected_mgmt_address_ipv4}'\n".encode("utf-8"))
                time.sleep(0.5)
                if self.mgmt_passthrough_ipv4_address == self.mgmt_address_ipv4:
                    # MGMT passthrough active, lets enable IPv6 aswell
                    self.tn.write(f"uci set network.mgmt.ip6addr='{self.mgmt_address_ipv6}'\n".encode("utf-8"))
    ...
    def configure_route(self, output):
        ...
        self.get_ready()
        changes_network = 0
        if "config route" not in output or ("option comment 'passthrough_on'" not in output and "option comment 'passthrough_off'" not in output):
            self.logger.info("❌ Route `mgmt` is missing, creating it...")
            self.logger.info(f"Route option target '{ipaddress.IPv4Interface(self.mgmt_address_ipv4).network}'")
            self.tn.write(b"uci add network route\n")
            time.sleep(0.5)
            self.tn.write(f"uci set network.@route[-1].interface='{self.mgmt_interface_interface}'\n".encode("utf-8"))
            time.sleep(0.5)

            if self.mgmt_passthrough_ipv4_address == self.mgmt_address_ipv4:
                self.tn.write(f"uci set network.@route[-1].target='{ipaddress.IPv4Interface(self.mgmt_address_ipv4).network}'\n".encode("utf-8"))
                time.sleep(0.5)
                self.tn.write(f"uci set network.@route[-1].gateway='{self.mgmt_passthrough_ipv4_gateway}'\n".encode("utf-8"))
                time.sleep(0.5)
                self.tn.write(b"uci set network.@route[-1].comment='passthrough_on'\n")
                time.sleep(0.5)
            else:
                self.tn.write(f"uci set network.@route[-1].target='{ipaddress.IPv4Interface(self.mgmt_passthrough_ipv4_address).network}'\n".encode("utf-8"))
                time.sleep(0.5)
                self.tn.write(f"uci set network.@route[-1].gateway='{self.mgmt_gw_ipv4}'\n".encode("utf-8"))
                time.sleep(0.5)
        ...
        for line in output.splitlines():
            match = re.search(r"network.@route\[(\d+)\].comment='(passthrough_on|passthrough_off)'", line)
            if match:
                ...
                if passthrough_status == "passthrough_on":
                    if self.mgmt_passthrough_ipv4_address != self.mgmt_address_ipv4:
                        ...
                        self.tn.write(f"uci set network.@route[{route_index}].target='{ipaddress.IPv4Interface(self.mgmt_passthrough_ipv4_address).network}'\n".encode("utf-8"))
                        time.sleep(0.5)
                        self.tn.write(f"uci set network.@route[{route_index}].gateway='{self.mgmt_gw_ipv4}'\n".encode("utf-8"))
                        time.sleep(0.5)
                        self.tn.write(f"uci set network.@route[{route_index}].comment='passthrough_off'\n".encode("utf-8"))
                        time.sleep(0.5)
                        self.tn.write(b"uci commit network\n")
                        time.sleep(0.5)
                        changes_network = 1
                    ...
                    if self.mgmt_passthrough_ipv4_address == self.mgmt_address_ipv4:
                        ...
                        self.tn.write(f"uci set network.@route[{route_index}].target='{ipaddress.IPv4Interface(self.mgmt_address_ipv4).network}'\n".encode("utf-8"))
                        time.sleep(0.5)
                        self.tn.write(f"uci set network.@route[{route_index}].gateway='{self.mgmt_passthrough_ipv4_gateway}'\n".encode("utf-8"))
                        time.sleep(0.5)
                        self.tn.write(f"uci set network.@route[{route_index}].comment='passthrough_on'\n".encode("utf-8"))
                        time.sleep(0.5)
                        self.tn.write(b"uci commit network\n")
                        time.sleep(0.5)
                        changes_network = 1
    ...
    def check_br_lan_ports(self):
        ...
        brlan_match = re.search(r"(config device[\s\S]+?option name 'br-lan'[\s\S]+?)(?=config|\Z)", output)
        ...
        if brlan_match:
            ...
            if eth0_in_brlan or not eth2_in_brlan:
                ...
                if eth0_in_brlan:
                    ...
                    self.tn.write(f"uci del_list network.@device[-1].ports='{self.mgmt_interface_device}'\r\n".encode("utf-8"))
                    time.sleep(0.5)
    ...
    def routes_and_dns(self):
        ...
        domains = "downloads.openwrt.org"  # Multiple domains separated by space or comma
        ...
        self.logger.info(f"➕ Adding temporary route to {self.packet_repository_dns_server}...")
        self.tn.write(b"uci add network route\r\n")
        time.sleep(0.5)
        self.tn.write(f"uci set network.@route[-1].interface='{self.mgmt_interface_interface}'\r\n".encode("utf-8"))
        time.sleep(0.5)
        self.tn.write(f"uci set network.@route[-1].target='{self.packet_repository_dns_server}/32'\r\n".encode("utf-8"))
        time.sleep(0.5)
        if self.mgmt_passthrough_ipv4_address == self.mgmt_address_ipv4:
            self.tn.write(f"uci set network.@route[-1].gateway='{self.mgmt_passthrough_ipv4_gateway}'\r\n".encode("utf-8"))
            time.sleep(0.5)
        ...
        for domain in domain_list:
            self.logger.info(f"[🔍] Resolving {domain} using {self.packet_repository_dns_server} via Telnet...")
            self.tn.write(f"nslookup {domain} {self.packet_repository_dns_server}\r\n".encode("utf-8"))
            time.sleep(2)  # Wait for response
            nslookup_output = self.tn.read_very_eager().decode("utf-8")

            # Extract IPv4 addresses
            resolved_ips = set(re.findall(r"Address:\s+(\d+\.\d+\.\d+\.\d+)", nslookup_output))  # Remove duplicates
            resolved_ips.discard(self.packet_repository_dns_server)  # Remove the nameserver itself

            if not resolved_ips:
                self.logger.info(f"❌ No IPv4 addresses found for {domain}.")
            else:
                self.logger.info(f"✅ Found IPv4 addresses for {domain}: {', '.join(resolved_ips)}")
        ...
        existing_hosts = re.findall(r"(\d+\.\d+\.\d+\.\d+)\s+(\S+)\s+#\s+opkg", hosts_output)
        for ip, domain in existing_hosts:
            ...
            if domain not in domain_list or ip not in ipv4_addresses.get(domain, []):
                self.logger.info(f"❌ Removing {ip} from /etc/hosts (domain {domain} no longer in domain_list or IP changed)...")
                self.tn.write(f"sed -i '/{ip} {domain} # opkg/d' /etc/hosts\r\n".encode("utf-8"))
                time.sleep(0.5)
                changes_hosts = 1
        ...
        for line in network_routes:
            match_target = re.search(r"network.@route\[(\d+)\].target='(\d+\.\d+\.\d+\.\d+/32)'", line)
            match_comment = re.search(r"network.@route\[(\d+)\].comment='(.+)'", line)
            match_gateway = re.search(r"network.@route\[(\d+)\].gateway='(\d+\.\d+\.\d+\.\d+)'", line)
        ...
        for route_index, route_data in existing_routes.items():
            ...
            if "-opkg" in route_comment:
                ...
                if stripped_comment not in domain_list or route_ip not in [ip + "/32" for ip in ipv4_addresses.get(stripped_comment, [])]:
                    self.logger.info(f"❌ Marking route {route_ip} (index {route_index}, comment: {route_comment}) "
                        "for removal (domain not in domain_list or IP changed)...")
                    routes_to_delete.append(int(route_index))
                if (route_gateway != expected_gateway) and stripped_comment in domain_list:
                    self.logger.info(f"🔄 Updating gateway for route {route_ip} (index {route_index}, comment: {route_comment}) "
                                    f"from {route_gateway} to {expected_gateway}...")
                    self.tn.write(f"uci set network.@route[{route_index}].gateway='{expected_gateway}'\r\n".encode("utf-8"))
                    time.sleep(0.5)
                    self.tn.write(b"uci commit network\r\n")
                    time.sleep(0.5)
                    changes_routes = 1
        ...
        routes_to_delete = sorted(set(routes_to_delete), reverse=True)  # unique + descending
        for route_index in routes_to_delete:
            self.logger.info(f"❌ Removing outdated route at index {route_index}...")
            self.tn.write(f"uci delete network.@route[{route_index}]\r\n".encode("utf-8"))
            time.sleep(0.5)
            self.tn.write(b"uci commit network\r\n")
            time.sleep(0.5)
            changes_routes = 1
        ...
        for domain, ips in ipv4_addresses.items():
            for ip in ips:
                if f"{ip} {domain} # opkg" not in hosts_output:
                    ...
                    self.tn.write(f"echo '{ip} {domain} # opkg' >> /etc/hosts\r\n".encode("utf-8"))
                    time.sleep(0.5)
                    changes_hosts = 1
        ...
        for domain, ips in ipv4_addresses.items():
            for ip in ips:
                ...
                if not route_exists:
                    ...
                    self.tn.write(f"uci set network.@route[-1].interface='{self.mgmt_interface_interface}'\r\n".encode("utf-8"))
                    time.sleep(0.5)
                    self.tn.write(f"uci set network.@route[-1].target='{ip}/32'\r\n".encode("utf-8"))
                    time.sleep(0.5)
                    if self.mgmt_passthrough_ipv4_address == self.mgmt_address_ipv4:
                        self.tn.write(f"uci set network.@route[-1].gateway='{self.mgmt_passthrough_ipv4_gateway}'\r\n".encode("utf-8"))
                        time.sleep(0.5)
                        ...
                        self.tn.write(f"uci set network.@route[-1].gateway='{self.mgmt_gw_ipv4}'\r\n".encode("utf-8"))
                        time.sleep(0.5)
                    self.tn.write(f"uci set network.@route[-1].comment='{domain}-opkg'\n".encode("utf-8"))
                    time.sleep(0.5)
                    self.tn.write(b"uci commit network\r\n")
                    time.sleep(0.5)
                    changes_routes = 1
        ...
        self.logger.info(f"❌ Searching for the temporary DNS route to {self.packet_repository_dns_server}...")
        ...
        if match:
            ...
            self.logger.info(f"✅ Found DNS route at index {route_index}, removing it...")
    ...
    def packet_update(self):
        ...
        prompt_lines = [line.strip() for line in prompt_output.split("\n") if "PROMPT_DETECT" not in line and line.strip()]
        ...
        if not upgradable_packages:
            ...
            self.logger.info(f"🔄 Upgrading {len(upgradable_packages)} packages: {', '.join(upgradable_packages)}")
    ...
    def packages_install(self):
        ...
        packages = [pkg.strip() for pkg in self.packages.replace(",", " ").split() if pkg.strip()]
        if not packages:
        ...
        not_installed_packages = [pkg for pkg in packages if pkg not in installed_packages]
        ...
        for package in not_installed_packages:
            self.logger.info(f"[🔎] Checking if `{package}` is available in the repository...")
            ...
            if any(line.startswith(package + " - ") for line in package_found.split("\n")):
...
class OpenWRT(vrnetlab.VR):
    def __init__(self, username, password, conn_mode, nics, hostname, packet_repository_domains, packet_repository_dns_server, packages, lan_ip, lan_netmask):
        ...
        self.vms = [ OpenWRT_vm(username, password, conn_mode, nics, hostname, packet_repository_domains, packet_repository_dns_server, packages, lan_ip, lan_netmask) ]
...
@click.option('--packages','-pcks', envvar='PACKAGES', required=False, help="eg tinc htop etc")
...
def args(tracing,username,password,connection_mode,nics,hostname,packet_repository_domains,packet_repository_dns_server,packages,lan_ip,lan_netmask):
        ...
        vr = OpenWRT(username, password, connection_mode, nics, hostname, packet_repository_domains, packet_repository_dns_server,packages, lan_ip, lan_netmask)
        vr.start()
