...
class cat9000v(vrnetlab.VR):
    def __init__(self, hostname, username, password, conn_mode):
        super().__init__(username, password)
        self.vms = [cat9000v_vm(hostname, username, password, conn_mode)]


class cat9000v_installer(cat9000v):
    """cat9000v installer

    Will start the cat9000v with a mounted iso to make sure that we get
    console output on serial, not vga.
    """

    def __init__(self, hostname, username, password, conn_mode):
        super().__init__(hostname, username, password, conn_mode)
        self.vms = [
            cat9000v_vm(hostname, username, password, conn_mode, install_mode=True)
        ]

    def install(self):
        self.logger.info("Installing cat9000v")
        cat9kv = self.vms[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--install", action="store_true", help="Install cat9000v")
    parser.add_argument("--hostname", default="cat9000v", help="Router hostname")
    parser.add_argument(
        "--connection-mode",
        default="tc",
    ...
    if args.install:
        vr = cat9000v_installer(
            args.hostname, args.username, args.password, args.connection_mode
        )
        vr.install()
    else:
        vr = cat9000v(args.hostname, args.username, args.password, args.connection_mode)
        vr.start()
